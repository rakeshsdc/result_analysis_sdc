...
def build_subject_failure_table(students, course_names):
    ...
    for code in course_names:
        appeared_n = appeared_counts.get(code, 0)
        failed_n = fail_counts.get(code, 0)
        pass_pct = round((appeared_n - failed_n) / appeared_n * 100, 2) if appeared_n else 0.0
        rows.append(
            {
                "Course Code": code,
...
def build_student_failure_details(students):
    """Sl.No, Name, No. of Papers Failed, Courses Failed (comma-separated codes)."""
    rows = []
    for s in students:
        failed_codes = [c["code"] for c in s["courses"] if c["grade"].upper() == "F"]
        if failed_codes:
            rows.append(
                {
                    "Name": s["name"],
                    "No. of Papers Failed": len(failed_codes),
                    "Courses Failed": ", ".join(failed_codes),
                }
            )
    rows.sort(key=lambda r: (-r["No. of Papers Failed"], r["Name"]))
    for i, r in enumerate(rows, start=1):
        r["Sl.No"] = i
    df = pd.DataFrame(rows)
    if not df.empty:
        df = df[["Sl.No", "Name", "No. of Papers Failed", "Courses Failed"]]
    return df
...
def build_summary_paragraphs(
    ...
    if fail_df is not None and not fail_df.empty and "Pass %" in fail_df.columns:
        max_pass_pct = fail_df["Pass %"].max()
        top_pass_courses = fail_df[fail_df["Pass %"] == max_pass_pct]
        course_list = ", ".join(
            f"{r['Course Code']} ({r['Course Title']})"
            for _, r in top_pass_courses.iterrows()
        )
        last_para_parts = [
            f"The course(s) with the highest pass percentage "
            f"({max_pass_pct}%) were: {course_list}."
        ]
    else:
        last_para_parts = []
    ...
    if fail_df is not None and not fail_df.empty and fail_df["Failed"].sum() > 0:
        ...
        last_para_parts.append("No student failed in any individual course.")

    paras.append(" ".join(last_para_parts))
...
def generate_docx_report(
    ...
    # ---------------- 5. Student-wise Failed Courses ----------------
    _add_heading(doc, "5. Student-wise Failed Courses")
    if student_fail_df is None or student_fail_df.empty:
        doc.add_paragraph("No student failed in any subject.")
    else:
        _add_table_from_df(doc, student_fail_df)

    # ---------------- 6. Course Code / Full Name Reference ----------------
    _add_heading(doc, "6. Course Code / Full Name Reference")
    _add_table_from_df(doc, legend_df)

    # ---------------- 7. Grade Distribution ----------------
    _add_heading(doc, "7. Grade Distribution")
    ...
    # ---------------- 8. Grade-wise List of Students ----------------
    _add_heading(doc, "8. Grade-wise List of Students")
    _add_table_from_df(doc, grade_wise_df)

    # ---------------- 9. Summary ----------------
    _add_heading(doc, "9. Summary")
...
st.write(
    "This tool reads a University **mark-cum-grade-statement PDF** (one page per "
    "student) and automatically builds a complete Result Analysis Report — a "
    "consolidated marks table, pass statistics, rank list, subject-wise and "
    "student-wise failure analysis, grade distribution, and a narrative summary — "
    "ready to view here and download as a formatted Word document with the "
    "college letterhead."
)
...
if "students" in st.session_state:
    ...
    # ---------------- 5. Student-wise Failed Courses ----------------
    st.subheader("5. Student-wise Failed Courses")
    student_fail_df = build_student_failure_details(students)
    if student_fail_df.empty:
        st.success("No student failed in any subject. 🎉")
    else:
        st.dataframe(student_fail_df, use_container_width=True, hide_index=True)

    # ---------------- 6. Course Code -> Full Name Legend ----------------
    st.subheader("6. Course Code / Full Name Reference")
    ...
    # ---------------- 8. Grade-wise List of Students ----------------
    st.subheader("8. Grade-wise List of Students")
    ...
    docx_bytes = generate_docx_report(
        ...
        top_df=top_df,
        fail_df=fail_df,
        student_fail_df=student_fail_df,
        legend_df=legend_df,
    ...
    st.info("👆 Fill in the examination details above and upload the mark-list PDF, then click **Generate Report**.")
