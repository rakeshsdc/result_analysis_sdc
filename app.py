...
with st.sidebar:
    ...
    college_logo = st.file_uploader("College Logo (optional)", type=["png", "jpg", "jpeg"])
...
if college_logo is not None:
    st.session_state["logo_bytes"] = college_logo.read()
...
if "students" in st.session_state:
    ...
    st.info("⬅️ Fill in the report details, optionally upload a college logo, and upload the mark-list PDF from the sidebar, then click **Generate Report**.")
